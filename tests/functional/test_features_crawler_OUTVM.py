...
class FeaturesCrawlerTests(unittest.TestCase):
    ...
    # USE_RUNNING_VM = True requires --pid=host on docker run command line in
    # Makefile
    ...
    def test_features_crawler_crawl_outvm_process(self):
        for crawler in self.crawlers:
            ...
            if FeaturesCrawlerTests.USE_RUNNING_VM is False:
                assert any('psvmi_test_init' in ProcessFeature._make(
                    item[1]).pname for item in crawler.crawl_processes())
