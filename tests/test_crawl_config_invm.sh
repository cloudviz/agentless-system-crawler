...
COUNT_MSG=`grep $UUID /tmp/test_crawl_config_invm | wc -l`
COUNT_FEATURES=`grep '^config' /tmp/test_crawl_config_invm | wc -l`

# 2 = METADATA + FEATURE
COUNT_TOTAL=`cat /tmp/test_crawl_config_invm | wc -l`
