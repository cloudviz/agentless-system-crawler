...
docker run -d --name test_crawl_cpu_container_1 ubuntu bash -c "echo bla >> /etc/ric_config; mkdir -p /bla; echo bla >> /bla/ble; sleep 120" 2> /dev/null > /dev/null
...
python2.7 ../config_and_metrics_crawler/crawler.py --crawlmode OUTCONTAINER \
	--features=cpu,memory,os,config,file,package,dockerinspect,_test_infinite_loop,dockerhistory,metric,load --crawlContainers $ID \
