...
if [ $COUNT == "1" ] && [ ! -f /var/log/crawler_container_logs/$HOST_IP/$NAME/var/log/input_file_name.log ] && [ ${COUNT_METRICS} -gt "0" ]
then
	:
	#echo 1
else
...
# By now the log should be there
COUNT=`grep -c $MSG /var/log/crawler_container_logs/$HOST_IP/$NAME/var/log/input_file_name_2.log`
...
# By now the container should be dead, and the link should be deleted
if [ $COUNT == "1" ] && [ ! -f /var/log/crawler_container_logs/$HOST_IP/$NAME/var/log/input_file_name_2.log ] && [ ${COUNT_METRICS} -gt "0" ]
then
	echo 1
else
	echo 0
