...
class MTGraphiteClient(object):
    ...
    def __init__(self, host_url, batch_send_every_t = 5,
    ...
    def _get_socket(self):
        ...
        while self.conn == None:
            try:
                ...
                self.conn = ssl.wrap_socket(self.socket, cert_reqs=ssl.CERT_NONE)
                self.conn.connect((self.host, int(self.port)))

                # We send this identifier message so that the server-side can identify
                # this specific crawler in the logs (its behind load-balancer so it never
                # sees our source-ip without this)
                ...
                logger.info("Sent authentication with mtgraphite, returned length = %d" % sent)
                if sent != len(authentication_message):
                    raise "failed to send tenant/passworad"
    ...
    def _write_messages_no_retries(self, msgset):
        ...
        messages_string.extend(bytearray(struct.pack('!I', len(msgset))))
        for m in msgset:
            if m == msgset[0]:
                # logger.debug the first message
                logger.debug(m.strip())
            messages_string.extend("1M")
            messages_string.extend(bytearray(struct.pack('!I', self.sequence)))
        ...
        while len_sent < len_to_send:
            t = time.time() * 1000
            logger.debug("About to write to the socket (already sent %d out of %d bytes)" % (len_sent, len_to_send))
        ...
        code = bytearray(chunk)[:2]
        logger.debug("MTGraphite server response of %s" % bytearray(chunk).strip())
        if code == "1A":
        ...
        while (not msg_sent and retries <= max_emit_retries):
            try:
                ...
                if retries <= max_emit_retries:
                    ...
                    logger.error("Could not connect to the mtgraphite server."
                                 "Retry in %f seconds." % (wait_time))
                    # The connection will be created again by
                    # _write_messages_no_retries
                    self.close()
                    time.sleep(wait_time)
                else:
                    logger.error("Bail out on sending to mtgraphite "
                                 "server")
                    raise
    ...
    def send_messages(self, messages):
        ...
        # Guarantee that messages is actually a list or tuple (should always be true)
        ...
        logger.debug("len(msgset)=%d, batch_every_n=%d, time=%d, "
                     ...
                     self.batch_send_every_n, time.time() , self.next_timeout))
        if len(messages) > 0:
            self.msgset.extend(messages)
        if (len(self.msgset) > self.batch_send_every_n or
            time.time() > self.next_timeout):
