...
Interface = namedtuple('Interface', ['ifname', 'ip_addresses'])
NetflowFeature = namedtuple('NetflowFeature', ['data'])
fprobe_out_dir = '/home/user1/fprobe-out'
...
class FprobeContainerCrawler(IContainerCrawler):
    ...
    def start_collector(self, user, socket, output_dir, watch_pid, metadata,
        ...
        #params = ['sh', '-c',
        #         '/usr/bin/python /usr/bin/socket-datacollector --sockfd ' + str(socket.fileno()) + ' --dir ' + output_dir + ' --filepattern ' + filepattern + ' --watch-pid ' + str(watch_pid) + ' --metadata ' + json.dumps(metadata) + ' --md-filter ip-addresses &']
        try:
    ...
    def start_netflow_collection(self, ifname, ip_addresses, container_id,
        ...
        """
        #pdb.set_trace()
        fprobe_user = kwargs.get('fprobe_user', 'user1')
        try:
    ...
    def remove_old_files(cls, **kwargs):
        ...
        """
        now = time.time()
        output_dir = kwargs.get('fprobe_output_dir', fprobe_out_dir)
    ...
    def _crawl_in_system(self):
        for ifname in netifaces.interfaces():
            ...
            yield Interface(ifname,self.get_ifaddresses(ifname))
